...
import { SearchInstituteId } from "./SearchInstituteID";

function PlayerInstitute() {
  ...
  useEffect(() => {
    fetchPlayers();
  }, [selectedPlayerId, selectedInstituteId]);

  const fetchPlayers = async () => {
    try {
      const res = await fetch(
        `http://localhost:5000/players/${selectedPlayerId}/institutes/${selectedInstituteId}`
      );
      if (!res.ok) {
  ...
  return (
    <div className="min-h-screen mx-auto px-10 py-6">
      ...
      <div className="px-32 flex gap-3">
        <SearchPlayerId
          selectedInstituteId={selectedInstituteId} // Pass down selected doctor ID
          setSelectedInstituteId={setSelectedInstituteId} // Pass down function to update doctor ID
      ...
      <h1>
        doc:{selectedInstituteId} player:{selectedPlayerId}
      </h1>
...
export default PlayerInstitute;
